...
public class MessageResource {
    ...
    public MessageResource(PatientRepository patientRepository ,UserRepository userRepository, MessageRepository messageRepository) {
    ...
    @PostMapping("/messages")
    public void createMessage(@RequestBody Message message) throws URISyntaxException {
        log.debug("REST request to save Message : {}", message);
        if (message.getId() != null) {
            throw new BadRequestAlertException("A new message cannot already have an ID", ENTITY_NAME, "idexists");
        }
        message.setDateTime(Instant.now());
        message.setSender(userRepository.findOneByCurrentUser());
        boolean isPatient =  SecurityUtils.isCurrentUserInRole(AuthoritiesConstants.PATIENT);
        if (isPatient) {
            Patient  p = patientRepository.findOneByUserIsCurrentUser();
            message.setReceiver(p.getMedecin().getUser());
        }

        else 
        message.setReceiver(userRepository.findOneById(message.getReceiver().getId()));
    ...
    public List<Message> getAllMessages() {
        ...
        return messageRepository.findAllByUserIsCurrentUser();
    }
